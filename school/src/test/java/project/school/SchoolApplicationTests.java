...
class SchoolApplicationTests {
	...
	void shouldReturnASortedPageOfSchools() {
		ResponseEntity<String> response = restTemplate.getForEntity("/schools?page=0&size=1&sort=rating,asc",
				String.class);
		assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
	...
	void shouldReturnASortedPageOfSchoolsWithNoParametersAndUseDefaultValues() {
		...
		JSONArray ratings = documentContext.read("$..rating");
		assertThat(ratings).containsExactly(9, 7, 5);
	}
	...
	void shouldDeleteAnExistingSchool() {
		ResponseEntity<Void> response = restTemplate.exchange("/schools/1", HttpMethod.DELETE, null, Void.class);
		assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NO_CONTENT);
	...
	void shouldNotDeleteASchoolThatDoesNotExist() {
		ResponseEntity<Void> deleteResponse = restTemplate.exchange("/schools/555", HttpMethod.DELETE, null,
				Void.class);
